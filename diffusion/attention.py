...
class GatedConv2dWithActivation(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,batch_norm=True, activation=nn.GELU()):
...
class BasicTransformerBlock(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=True):
        ...
        if fuser_type == "gatedSA":  # 一般都用的gatedSA
...
class SpatialTransformer(nn.Module):
    def __init__(self, in_channels, key_dim, value_dim, n_heads, d_head, depth=1, fuser_type=None, use_checkpoint=True):
        ...
        # self.proj_sem = zero_module(nn.Conv2d(768,
        #                                     query_dim,
        #                                     kernel_size=1,
        #                                     stride=1,
        #                                     padding=0))
        self.proj_sem = nn.Conv2d(768,
                                query_dim,
                                kernel_size=1,
                                stride=1,
                                padding=0)

        # self.gated_conv = GatedConv2dWithActivation(query_dim * 2,
        #                                             query_dim,
        #                                             kernel_size=3, 
        #                                             stride=1, 
        #                                             padding=1)

    def forward(self, x, context, objs, sem_feature):
        b, c, h, w = x.shape
        x_in = x
        x = self.norm(x)
        x = self.proj_in(x)
        x = rearrange(x, 'b c h w -> b (h w) c').clone()
        for block in self.transformer_blocks:
            x = block(x, context, objs)
        x = rearrange(x, 'b (h w) c -> b c h w', h=h, w=w).clone()

        sem_feature = torch.nn.functional.interpolate(sem_feature, (h, w), mode="nearest").clone()  # wzy
        sem_feature = self.proj_sem(sem_feature)  # wzy
        x = x + sem_feature  # wzy
        # x = torch.cat((x, sem_feature), dim=1) # wzy
        # x = self.gated_conv(x) # wzy
